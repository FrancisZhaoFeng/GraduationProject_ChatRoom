...
//		Query query = session.createQuery("from Manager as m where m.adminname=?");
//		query.setParameter(0, manager.getAdminname());
//		managers = query.list();
//		session.close();
		String hql="from Manager u where u.adminname='"+manager.getAdminname()+"' and u.password ='"+manager.getPassword()+"'";
		System.out.println(hql);
		managers = sessionFactory.getCurrentSession().createQuery(hql).list();
		return managers;
	}
	...
	public List<ChatRoom> selectChatRoom(String searchStr){
		...
		System.out.println(hql);
		chatRooms = sessionFactory.getCurrentSession().createQuery(hql).list();
		return chatRooms;
	}
	...
	public List<ChatRoomLog> selectChatRoomLog(String searchStr){
		...
		if(searchStr == null ||searchStr.equals("")){
			...
			hql="from ChatRoomLog where type!=4 and (senderid like '%"+searchStr+"%' or chatroomid like '%"+searchStr+"%' or sendtime like '%"+searchStr+"%' or sendtext like '%"+searchStr+"%' )";
		}
		System.out.println(hql);
		chatRoomLogs = sessionFactory.getCurrentSession().createQuery(hql).list();
		return chatRoomLogs;
	}
	...
	public List<ChatPerLog> selectChatPerLog(String searchStr){
		...
		if(searchStr == null ||searchStr.equals("")){
			...
			hql="from ChatPerLog where type!=4 and (senderid like '%"+searchStr+"%' or receiverid like '%"+searchStr+"%' or sendtime like '%"+searchStr+"%' or sendtext like '%"+searchStr+"%')";
		}
		System.out.println(hql);
...
//	}
	...
	public boolean updateChatRoom(ChatRoom chatRoom){
		try{
			sessionFactory.getCurrentSession().merge(chatRoom);
		}catch (Exception e) {
			e.printStackTrace();
			return false;
			// TODO: handle exception
	...
	public boolean updateChatRoomLog(ChatRoomLog chatRoomLog){
		try{
			sessionFactory.getCurrentSession().merge(chatRoomLog);
		}catch (Exception e) {
			e.printStackTrace();
			return false;
			// TODO: handle exception
	...
	public boolean updateChatPerLog(ChatPerLog chatPerLog){
		try{
			sessionFactory.getCurrentSession().merge(chatPerLog);
		}catch (Exception e) {
			e.printStackTrace();
			return false;
			// TODO: handle exception
