...
public class ChatService extends Service {
	...
	public void newMsgArrive(String str0, boolean isSelf) {
		...
		if (!ChatActivity.getIsActive() || mFriendId != id) {
			MainTabMsgPage.getInstance().onUpdateByUserinfo(FriendListInfo.getFriendListInfo().getUserFromId(id), msgEntity.getContent(), msgEntity.getTime(), true);
		} else {
			MainTabMsgPage.getInstance().onUpdateByUserinfo(FriendListInfo.getFriendListInfo().getUserFromId(id), msgEntity.getContent(), msgEntity.getTime(), false);
		}
