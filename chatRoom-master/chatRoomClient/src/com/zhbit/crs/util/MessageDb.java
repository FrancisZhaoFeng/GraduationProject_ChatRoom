...
public class MessageDb {
	...
	public void saveMsg(int masterId, int guestId, boolean isSelfBool, ChatEntity entity) {
		...
		mSqlDb.execSQL("create table if not exist _" + masterId + "_" + guestId + " (_index int primary key auto_increment," + "isSelf 	int," + "senderId int," + "senderAvatarId int," + "userName varchar(100)," + "gender int," + "time varchar(100)," + "content text," + "receiverId int)");
		mSqlDb.execSQL("insert into _" + masterId + "_" + guestId + " (isSelf,senderId,senderAvatarId,userName,gender,time,content,receiverId)" + " values(?,?,?,?,?,?,?,?)", new Object[] { isSelf, entity.getSenderId(), entity.getSenderAvatarid(), entity.getName(), entity.getSex(), entity.getTime(), entity.getContent(), entity.getReceiverId() });
	}

	public void getMsg(ArrayList<ChatEntity> chatList, ArrayList<Boolean> boolList, int masterId, int guestId) {
		mSqlDb.execSQL("create table if not exist _" + masterId + "_" + guestId + " (_index int primary key auto_increment," + "isSelf int," + "senderId int," + "senderAvatarId int," + "userName varchar(100)," + "gender int," + "time varchar(100)," + "content text," + "receiverId int)");
		Cursor curs = mSqlDb.rawQuery("SELECT * from _" + masterId + "_" + guestId + " ORDER BY _index DESC LIMIT " + mStartPoint + ", " + mReadLength, null);
		if (!curs.moveToFirst()) {
			return;
		}
		do {
...
//			Date time = curs.getString(curs.getColumnIndex("time"));
...
//			ChatEntity entity0 = new ChatEntity(senderId, senderAvatarId, userName, gender, time, content, receiverId);
