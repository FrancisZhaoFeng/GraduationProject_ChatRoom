...
public class DbSaveOldMsg {
	...
	private DbSaveOldMsg() {
		mSqlDb = mContext0.openOrCreateDatabase(mContext0.getFilesDir().getParent() + "/databases/" + DbName, Context.MODE_PRIVATE, null);
	...
	// masterId is the id of client user, guestId is the id of user this guy is
	// talking to
	public void saveMsg(int masterId, int guestId, boolean isSelfBool, ChatPerLog entity) {
		int isSelf;
		if (isSelfBool) {
			isSelf = 1;
		} else {
		...
		mSqlDb.execSQL("create table if not exists oldMsg_" + masterId + "_" + guestId + " (_index integer primary key, " + "isSelf int, " + "strEntity text)");
		mSqlDb.execSQL("insert into oldMsg_" + masterId + "_" + guestId + " (isSelf,strEntity)" + " values(?,?)", new Object[] { isSelf, strOfEntity });
	...
	public int getMsg(ArrayList<ChatPerLog> chatList, ArrayList<Boolean> boolList, int masterId, int guestId) {
		...
		mSqlDb.execSQL("create table if not exists oldMsg_" + masterId + "_" + guestId + " (_index integer primary key, " + "isSelf int, " + "strEntity text)");
		Cursor curs = mSqlDb.rawQuery("SELECT * from OldMsg_" + masterId + "_" + guestId + " ORDER BY _index DESC LIMIT " + mStartPoint + ", " + mReadLength, null);
		if (!curs.moveToFirst()) {
			return 0;
		} else {
...
//			ChatPerLog ent0 = new ChatEntity(strEnt0);
//			chatList.add(0, ent0);
...
//		} while (curs.moveToNext());
	...
	public void saveTabMsgItem(int id, List<TabMsgItemEntity> listOfEnt0) {
		mSqlDb.execSQL("create table if not exists oldTabMsg_" + id + " (_index integer primary key, " + "strEntity text)");

		mSqlDb.execSQL("delete from oldTabMsg_" + id);

		for (TabMsgItemEntity ent0 : listOfEnt0) {
			String strOfEntity = ent0.toString();
			mSqlDb.execSQL("insert into oldTabMsg_" + id + " (strEntity)" + " values(?)", new Object[] { strOfEntity });
		}
	}
	...
	public void getTabMsgItem(int id, ArrayList<TabMsgItemEntity> msgItemList) {
		...
		Cursor curs = mSqlDb.rawQuery("SELECT * from OldTabMsg_" + id + " ORDER BY _index DESC", null);
		if (!curs.moveToFirst()) {
			return;
		}
	...
	public void saveFrdReqNotif(int id, List<FrdReqNotifItemEntity> listOfEnt0) {
		mSqlDb.execSQL("create table if not exists oldFrdReqNotif_" + id + " (_index			integer		primary key, " + "strEntity			text)");

		mSqlDb.execSQL("delete from oldFrdReqNotif_" + id);

		for (FrdReqNotifItemEntity ent0 : listOfEnt0) {
			String strOfEntity = ent0.toString();
			mSqlDb.execSQL("insert into oldFrdReqNotif_" + id + " (strEntity)" + " values(?)", new Object[] { strOfEntity });
		}
	}
	...
	public void getFrdReqNotif(int id, ArrayList<FrdReqNotifItemEntity> msgItemList) {
		...
		Cursor curs = mSqlDb.rawQuery("SELECT * from oldFrdReqNotif_" + id + " ORDER BY _index DESC", null);
		if (!curs.moveToFirst()) {
			return;
		}
	...
	public void saveUnreadMsgs(int myId, int friendId, int unreadAm) {
		mSqlDb.execSQL("create table if not exists oldUnreadMsgs_" + myId + " (_friendId integer, " + "unreadAmount integer)");

		mSqlDb.execSQL("delete from oldUnreadMsgs_" + myId + " where _friendId=?", new Object[] { friendId });

		mSqlDb.execSQL("insert into oldUnreadMsgs_" + myId + " (_friendId,unreadAmount) values(?,?)", new Object[] { friendId, unreadAm });
	}

	public int getUnreadMsgs(int myId, int friendId) {
		mSqlDb.execSQL("create table if not exists oldUnreadMsgs_" + myId + " (_friendId integer, " + "unreadAmount integer)");

		Cursor curs = mSqlDb.rawQuery("SELECT * from oldUnreadMsgs_" + myId + " where _friendId=" + friendId, null);
		if (!curs.moveToFirst()) {
			return 0;
		}
